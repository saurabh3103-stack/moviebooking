...
<body>
    ...
    <div class="container mx-auto mt-10">
        <div class="card bg-none shadow-lg p-4 rounded-3" style="background: #001232; border: 1px solid #fff;">
            <h4 class="mb-3 text-white">Booking Preview: {{ $bookingdata['movie_name'] }}</h4>

            <!-- Booking Details -->
            <div class="mb-3 text-white">
                <p><strong>Movie:</strong> {{ $bookingdata['movie_name'] }}</p>
                <p><strong>Date:</strong> {{ $bookingdata['show_date'] }}</p>
                <p><strong>Time:</strong> {{ $bookingdata['show_time'] }}</p>
                <p><strong>Number of Tickets:</strong> {{ $bookingdata['tickets'] }}</p>
                <p><strong>Price per Ticket:</strong> ₹{{ $bookingdata['ticket_price'] }}</p>
                <p><strong>Total Price:</strong> ₹{{ $bookingdata['total_price'] }}</p>
                <p class="text-red-400"><small>This booking will expire at {{ $bookingdata['expires_at'] }}</small></p>
            </div>

            <!-- Hidden Form to Confirm Booking -->
            <form action="{{route('ticketBook')}}" method="POST">
                @csrf
                <input type="hidden" name="user_id" value="{{ $bookingdata['user_id'] }}">
                <input type="hidden" name="movie_id" value="{{ $bookingdata['movie_id'] }}">
                <input type="hidden" name="show_date" value="{{ $bookingdata['show_date'] }}">
                <input type="hidden" name="show_time" value="{{ $bookingdata['show_time'] }}">
                <input type="hidden" name="tickets" value="{{ $bookingdata['tickets'] }}">
                <input type="hidden" name="ticket_price" value="{{ $bookingdata['ticket_price'] }}">
